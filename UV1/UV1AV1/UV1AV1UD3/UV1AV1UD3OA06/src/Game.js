...
BasicGame.Game.prototype = {
    ...
    createRandomString: function(itens, num) {
        var ïtem = [];
    ...
    showTextoIntro: function() {
        var tutorialText = this.add.sprite( this.world.centerX+65, 120, 'initialText');
            tutorialText.alpha = 0;
            tutorialText.anchor.set(0.5, 0.5);

        this.groupIntro.add(tutorialText);

        var placaTexto = this.add.sprite(this.world.centerX, 430, 'placaLisa');
            placaTexto.alpha = 0;
            placaTexto.anchor.set(0.5, 0.5);

        this.groupIntro.add(placaTexto);
        ...
        this.createDelayTime( 13000, function() {
            ...
            this.placaAtual = this.exibePlaca();

            this.groupIntro.add(this.placaAtual);

            this.txtPlacaAtual = this.exibeTextoPlaca("LAMA");

            this.groupIntro.add(this.txtPlacaAtual);

            this.botao1 = this.exibeBotao(this.world.centerX-250, 50);
            this.botao2 = this.exibeBotao(this.world.centerX-80, 50);
            this.botao3 = this.exibeBotao(this.world.centerX+100, 50);

            this.groupIntro.add(this.botao1);
            this.groupIntro.add(this.botao2);
            this.groupIntro.add(this.botao3);

            this.txtBotao1 = this.exibeTextoBotao(this.world.centerX-250, 50, "4");
            this.txtBotao2 = this.exibeTextoBotao(this.world.centerX-80, 50, "2");
            this.txtBotao3 = this.exibeTextoBotao(this.world.centerX+100, 50, "3");

            this.groupIntro.add(this.txtBotao1);
            this.groupIntro.add(this.txtBotao2);
            this.groupIntro.add(this.txtBotao3);

            this.createDelayTime( 9500, function() {
                this.arrow = this.add.sprite(this.world.centerX, this.world.centerY+50, "mouseIntro");
                this.arrow.anchor.set(0.5,0.5);

                this.groupIntro.add(this.arrow);

                this.add.tween(this.arrow).to({x:this.world.centerX-120, y: 150}, 1200, Phaser.Easing.Linear.None, true).onComplete.add(this.showClickEffect,this);
                this.createDelayTime( 3500, function() {
                    this.transformarDoce('doceIntro');

                    this.groupIntro.add(this.doceAtual);
                })
            });
        });
        ...
        this.createDelayTime( this.TEMPO_INTRO, function() {
            ...
            this.add.tween(this.doceAtual).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);
            this.add.tween(this.tutorialPlacar).to({y: -300}, 1000, Phaser.Easing.Linear.None, true, 500).onComplete.add(this.initGame, this);
        });
    },

    exibePlaca: function() { 
        var placa = this.add.sprite(this.world.centerX, 430, 'placaLisa');
            placa.alpha = 0;
            placa.anchor.set(0.5, 0.5);
            this.add.tween(placa).to({alpha: 1}, 1500, Phaser.Easing.Linear.None, true);
        return placa;
    },

    exibeTextoPlaca: function(palavra) {
        var palavraAtual = this.add.bitmapText(0, 400, "JandaManateeSolid", palavra, 45);
            palavraAtual.alpha = 0;
            palavraAtual.x = this.world.centerX - (palavraAtual.width / 2);
            this.add.tween(palavraAtual).to({alpha: 1}, 500, Phaser.Easing.Linear.None, true);
        return palavraAtual;
    },

    exibeBotao: function(x, y) { 
        var botao = this.add.sprite(x, y, 'efeitoBotao');
            botao.animations.add('idle', null, 18, false);
            botao.animations.play('idle');
            botao.scale.set(1, 1);
        return botao;
    },

    exibeTextoBotao: function(x, y, numero) {
        var palavraAtual = this.add.bitmapText(x+70, y+30, "JandaManateeSolid", numero, 50);
            palavraAtual.alpha = 0;
            palavraAtual.tint = 0x0372e0;
            this.add.tween(palavraAtual).to({alpha: 1}, 500, Phaser.Easing.Linear.None, true);
        return palavraAtual;
    },

    showClickEffect: function() { 
        var botao = this.add.sprite(this.world.centerX-170, 90, 'efeitoClique');
            botao.animations.add('idle', null, 18, false);
            botao.animations.play('idle');
            botao.scale.set(1, 1);
    },
    ...
    hideButtons: function() { 
        this.add.tween(this.botao1).to({alpha: 0}, 1000, Phaser.Easing.Linear.None, true);
        this.add.tween(this.botao2).to({alpha: 0}, 1000, Phaser.Easing.Linear.None, true);
        this.add.tween(this.botao3).to({alpha: 0}, 1000, Phaser.Easing.Linear.None, true);
        this.add.tween(this.txtBotao1).to({alpha: 0}, 1000, Phaser.Easing.Linear.None, true);
        this.add.tween(this.txtBotao2).to({alpha: 0}, 1000, Phaser.Easing.Linear.None, true);
        this.add.tween(this.txtBotao3).to({alpha: 0}, 1000, Phaser.Easing.Linear.None, true);
    },
    ...
    transformarDoce: function(nomeDoce) { 
        this.hidePlaca();
        //this.doceAtual = this.add.sprite(this.world.centerX-90, 430, nomeDoce);
        this.doceAtual = this.add.sprite(this.world.centerX-90, 430, nomeDoce);
        this.doceAtual.alpha = 0;
        this.add.tween(this.doceAtual).to({alpha: 1}, 2000, Phaser.Easing.Linear.None, true);

        var fumaca = this.add.sprite(this.world.centerX-90, 430, 'efeitoFumaca');
            fumaca.animations.add('idle', null, 18, false);
            fumaca.animations.play('idle');
            fumaca.scale.set(1, 1);
    },

    hidePlaca: function() {
        this.add.tween(this.placaAtual).to({alpha: 0}, 200, Phaser.Easing.Linear.None, true);
        this.add.tween(this.txtPlacaAtual).to({alpha: 0}, 1000, Phaser.Easing.Linear.None, true);
    },
    ...
    gameOverMacaco: function() {
        ...
        this.add.tween(bg).to({alpha: 1}, 500, Phaser.Easing.Linear.None, true, 9000);
        this.add.tween(animal).to({alpha: 1}, 1000, Phaser.Easing.Linear.None, true, 9500).onComplete.add(function() {
    ...
    clearButtons: function() {
        ...
        for(var i = 0; i < this.numerosOpt.length; i++) {
            this.add.tween(this.numerosOpt[i]).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true).onComplete.add(function(elem) {
                elem.destroy();
            })
        }
    },
    ...
    showCriancasFixo: function() {
        this.polyFixo = this.createAnimation( 680, 360, 'polyFixo', 1,1);
    },

    // resumo inicial
    showTextResumo: function() {
        this.sound.play("soundResumo");
        //this.resumoAnim = this.createAnimationStop(this.world.centerX-230, 100,'animacaoResumo',1,1);

        var tutorialText = this.add.sprite( this.world.centerX, 120, 'txtResumo');
            tutorialText.anchor.set(0.5, 0.5);
    ...
    initLevel1: function() {
        this.countClick=0;
        this.palavras = ["FLOR", "RAIZ", "SOL"];
        this.palavraLevel = this.createRandomString(this.palavras, 1);
        this.correto = String(this.palavraLevel.length);
        this.errados = ["1","2","5"];
        var _letters  = this.createRandomItens(this.errados, 2);
        _letters.push(this.correto);
        ...
        this.placaAtual = this.exibePlaca();
        this.txtPlacaAtual = this.exibeTextoPlaca(this.palavraLevel);

        this.createDelayTime( (this.isWrong)?500:2000, function() {
            this.buttons = [];
            this.buttons.push(this.createButton(this.world.centerX-180, 70, this.world.centerX-250, 5, _letters[0], (_letters[0] == this.correto), 100));
            this.buttons.push(this.createButton(this.world.centerX, 70, this.world.centerX-80, 5, _letters[1], (_letters[1] == this.correto), 100));
            this.buttons.push(this.createButton(this.world.centerX+180, 70, this.world.centerX+100, 5, _letters[2], (_letters[2] == this.correto), 100));
        });
    },

    initLevel2: function() {
        this.countClick =0;
        this.palavras = ["GALHO", "FOLHA", "RIACHO"];
        this.palavraLevel = this.createRandomString(this.palavras, 1);
        this.correto = String(this.palavraLevel.length);
        this.errados = ["4","7","8","9"];
        var _letters  = this.createRandomItens(this.errados, 2);
        _letters.push(this.correto);
        ...
        this.createDelayTime( (this.isWrong)?500:3000, function() {
            this.buttons = [];
            this.buttons.push(this.createButton(this.world.centerX-180, 70, this.world.centerX-250, 5, _letters[0], (_letters[0] == this.correto), 100));
            this.buttons.push(this.createButton(this.world.centerX, 70, this.world.centerX-80, 5, _letters[1], (_letters[1] == this.correto), 100));
            this.buttons.push(this.createButton(this.world.centerX+180, 70, this.world.centerX+100, 5, _letters[2], (_letters[2] == this.correto), 100));
    ...
    initLevel3: function() {
        this.countClick=0;
        this.palavras = ["ÁRVORE", "PLANTA", "CAMINHO"];
        this.palavraLevel = this.createRandomString(this.palavras, 1);
        this.correto = String(this.palavraLevel.length);
        this.errados = ["4","5","8","9"];
        var _letters  = this.createRandomItens(this.errados, 2);
        _letters.push(this.correto);
    ...
    createButton: function( x, y, x2, y2, letter, right, time, canInteract) {
        ...
        var btn;
        if(right) {
            btn = this.add.button(x,y, 'fundoBotao', (_canInteract)?this.clickRightButton:null, this, 0,0,0);
            this.numerosOpt.push(this.exibeTextoBotao(x2, y2, letter));
        } else {
            btn = this.add.button(x,y, 'fundoBotao', (_canInteract)?this.clickWrongButton:null, this, 0,0,0);
            this.numerosOpt.push(this.exibeTextoBotao(x2, y2, letter));
        }
    ...
    clickRightButton: function(target) {
        ...
        if(this.countClick==0)
        {
            ...
            this.clearButtons();
            this.doces = ["mostraDoce1", "mostraDoce2", "mostraDoce3", "mostraDoce4"];
            var nomeDoce = this.createRandomItens(this.doces, 1);
            this.transformarDoce(nomeDoce);
