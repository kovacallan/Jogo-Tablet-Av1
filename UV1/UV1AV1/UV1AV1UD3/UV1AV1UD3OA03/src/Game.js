...
BasicGame.Game.prototype = {
    ...
    onButtonOver: function(elem) {
        this.add.tween(elem.scale).to({x: elem.initScale+0.1, y: elem.initScale+0.1}, 100, Phaser.Easing.Linear.None, true);
    },
    //fixa
    onButtonOut: function(elem) {
        this.add.tween(elem.scale).to({x: elem.initScale, y: elem.initScale}, 100, Phaser.Easing.Linear.None, true);
    },
    ...
    clearButtons: function(isWrong) {

        for(var i = 0; i < this.buttons.length; i++) {
            if(this.buttons[i].children.length > 0) {
                this.add.tween(this.buttons[i]).to({y: 1200}, 2000, Phaser.Easing.Linear.None, true).onComplete.add(function(elem) {
                    
                    this.add.tween(elem.getChildAt(0)).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);

                }, this);
            } else {
                if(isWrong) {
                    this.add.tween(this.buttons[i]).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true, 1200);
                } else {
                    this.add.tween(this.buttons[i]).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true, 2000);
                }
            }
        }
    },
    ...
    hideLevel: function(callback) {
        ...
        this.add.tween(this.imagePlacar).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);
    ...
    createScene: function() {//finished
        ...
        this.add.sprite( -268, -151, 'background');
    ...
    showLiveTutorial: function() {
        ...
        this.imagePlacar = this.add.sprite(this.world.centerX, 100, "quadros3");
        this.imagePlacar.anchor.set(0.5,0.5);
        this.imagePlacar.scale.set(0.7,0.7);

        this.groupIntro.add(this.imagePlacar);

        this.buttons = [];
        this.buttons.push( this.createButton(this.world.centerX-220, 530, "foto1-2", true, 100, 0.5, false) );
        this.buttons.push( this.createButton(this.world.centerX    , 550, "foto1-1", true, 300, 0.5, false) );
        this.buttons.push( this.createButton(this.world.centerX+220, 530, "foto1-3", true, 500, 0.5, false) );
        ...
        this.createDelayTime( 7000, function() {
            ...
            var t1 = this.add.tween(this.arrow)
                        .to({x:500, y: 390},  500, Phaser.Easing.Linear.None, false)
                        .to({x:340, y:  90}, 1200, Phaser.Easing.Linear.None, false)
                        .to({x:280, y: 370},  500, Phaser.Easing.Linear.None, false)
                        .to({x:500, y:  90}, 1200, Phaser.Easing.Linear.None, false)
                        .to({x:725, y: 370},  500, Phaser.Easing.Linear.None, false)
                        .to({x:665, y:  90}, 1200, Phaser.Easing.Linear.None, false)
            t1.start();

            this.add.tween(this.buttons[1].getChildAt(0)).to({x:-163, y: -420}, 1200, Phaser.Easing.Linear.None, true, 500);
            this.add.tween(this.buttons[0].getChildAt(0)).to({x: 220, y: -400}, 1200, Phaser.Easing.Linear.None, true, 2200);
            this.add.tween(this.buttons[2].getChildAt(0)).to({x: -60, y: -400}, 1200, Phaser.Easing.Linear.None, true, 3900);

            this.createDelayTime(6000, this.showFinishedLiveTutorial);
    ...
    showNextLevel: function() {
        ...
        switch(this.currentLevel) {
            case 1:
                ...
                this.initLevel( ["foto2-1", "foto2-2", "foto2-3"], 1, 3, (this.isWrong)?500:4000, 0.7, 2);
            break;
            case 2:
                if(!this.isWrong) {
                    this.sound.play("soundP2" );
                }
                this.initLevel( ["foto3-1", "foto3-2", "foto3-3", "foto3-4"], 2, 4,(this.isWrong)?500:5000, 0.58, 4);
            break;
            case 3:
                if(!this.isWrong) {
                    this.sound.play( "soundP3" );
                }
                this.initLevel( ["foto4-1", "foto4-2", "foto4-3", "foto4-4", "foto4-5"], 3, 5,(this.isWrong)?500:4000, 0.54, 5);
            break;
        }
        this.isWrong = false;
    },
    ...
    showPlacar: function(num) {
        this.currentPlacar = num;
        this.imagePlacar  = this.add.sprite(this.world.centerX, 250, "quadros" + num);

        this.posX = [null,null,null,
            [272, 433, 595],
            [264, 388, 510, 634],
            [260,360,459,558,657]
        ];
        this.posY = [null,null,null,202, 212,225];
        this.posW = [null,null,null,130, 100, 80];
        this.posH = [null,null,null, 90,  70, 50];

        this.imagePlacar.anchor.set(0.5,0.5);
        this.imagePlacar.scale.set(0.7,0.7);
    },

    initLevel: function(itens, question, placar, time, scale, corrects) {
        ...
        this.showPlacar(placar);

        this.localErrors = 2;
        this.itensRight = corrects;
        
        // fixo - criar sistema de botoes dentro do array
        this.buttons = [];

        var xIni = Math.round(this.itens.length*0.5);
        
        var count = 0;
        for(var i = 0; i < this.itens.length; i++) {
            if(count >= corrects) {
                this.buttons.push( this.createFinalPicture(this.itens[i], scale) );
            } else {
                if(placar == 5) {
                    this.buttons.push( this.createButton(this.world.centerX-20 - 80*xIni + 160*i, 580, this.itens[i], true, time+i*200, scale) );
                } else {
                    this.buttons.push( this.createButton(this.world.centerX-120 - 80*xIni + 180*i, 580, this.itens[i], true, time+i*200, scale) );
                }
            }
            count++;
        }
    ...
    createFinalPicture: function(name, scale) {

        var px1 = 0,
            px2 = this.posW[this.currentPlacar],
            py1 = this.posY[this.currentPlacar],
            py2 = this.posH[this.currentPlacar],
            n   = parseInt(name.substring(name.length-1))-1;

        
        px1 = this.posX[this.currentPlacar][n];

        var spr = this.add.sprite(px1+px2*0.5, py1+py2, "fotos", name);
    ...
    onDragStartFoto: function(elem) {
        this.initPos = new Phaser.Point(elem.x,elem.y);
        ...
        /*var px1 = 0,
            ...
            y1 = py1-20;
            y2 = py1 + (py2+30);
            for(var i = 0; i < this.posX[this.currentPlacar].length; i++) {
                px1 = this.posX[this.currentPlacar][i];
                x1 = px1;
                x2 = px1 + px2;
                cor = 0xff0000;
    ...
    onDragStopFoto: function(elem) {
        ...
        if(this.input.y > y1 && this.input.y < y2) {
            for(var i = 0; i < this.posX[this.currentPlacar].length; i++) {
                ...
                if(this.input.x > px1 && this.input.x < px1 + px2) {
                    if(i == elem.letter) {
                        console.log("right");
                        _goBack = false;
                        this.itensRight--;
                        ...
                        this.gotoRightPos(elem, px1+px2*0.5, py1+py2);

                        if(this.itensRight <= 0) {
        ...
        if(_goBack) {
            this.add.tween(elem).to({x:this.initPos.x, y: this.initPos.y}, 200, Phaser.Easing.Linear.None, true);
        }
        this.initPos = null;
    },

    gotoRightPos: function(elem,x,y) {
        elem.inputEnabled = false;
        var gl = elem.toGlobal({x:0,y:0});
        
        elem.parent = this.game.world;
        elem.x = gl.x;
        elem.y = gl.y;

        this.add.tween(elem).to({ x: x, y: y }, 150, Phaser.Easing.Linear.None, true);
        this.add.tween(elem.scale).to({x: elem.initScale, y: elem.initScale}, 150, Phaser.Easing.Linear.None, true);
    },

    //criacao de botao de resposta - manter estrutura
    createButton: function( x, y, imagem, right, time, scale, canInteract) {

        var _canInteract = (canInteract==null||canInteract==undefined) ? true : false;
        
        var btn = this.add.sprite(x,900, 'buttonModel');
        ...
        var spr = this.add.sprite(0,-130, "fotos", imagem);
        spr.inputEnabled = true;
        spr.anchor.set(0.5,1);
        spr.scale.set(scale,scale);
        spr.initScale = scale;
        spr.letter = parseInt(imagem.substring(imagem.length-1))-1;

        btn.addChild(spr);

        if(_canInteract) {
            spr.events.onInputOver.add(this.onButtonOver, this);
            spr.events.onInputOut.add(this.onButtonOut, this);
        }
        ...
        this.add.tween(btn).to({y: y}, 500, Phaser.Easing.Back.Out, true, time).onComplete.add(function() {
            if(_canInteract) {
                ...
                spr.events.onDragStart.add(this.onDragStartFoto, this);
                spr.events.onDragStop.add(this.onDragStopFoto, this);
            }
        }, this);
