...
BasicGame.Game.prototype = {
	...
	create: function () {
        ...
        this.TEMPO_INTRO = 15000;
        this.TEMPO_ERRO1 = 6700;
        this.TEMPO_RESUMO = 17500;
        /**************************** CONSTANTES GERAIS FIXAS ************************************************/

        /**************************** CONSTANTES JOGO ATUAL ************************************************/
        this.LETTER_SPACING = 60;
        this.UNDERLINE_SPACING = 15;
        /**************************** CONSTANTES JOGO ATUAL ************************************************/
    ...
    gameOverMacaco: function() {
        ...
        this.add.tween(bg).to({alpha: 1}, 500, Phaser.Easing.Linear.None, true, 8500);
        this.add.tween(animal).to({alpha: 1}, 1000, Phaser.Easing.Linear.None, true, 9000).onComplete.add(function() {
    ...
    showName: function(y, name) {
        ...
        for(var i = 0; i < name.length; i++) {
            ...
            var px = this.world.centerX - name.length*20 + last;
            //console.log(px, last);

            var py = (name[i] == "_") ? y+this.UNDERLINE_SPACING : y;

            var lastLetter = this.addLetter(px,py, name[i]);
            var nSize = lastLetter[0].width;
            if(name[i] == "_") {
                this.nameCorrectPos = px;
                var lSize = this.add.bitmapText(0,0, "JandaManateeSolid", this.nameCorrect[i], 70);
                nSize = lSize.width;
                lSize.destroy();
    ...
    addLetter: function(x,y, letter) {
        ...
        var shadow = this.add.bitmapText(x+4,y+4, "JandaManateeSolid", letter, 70);
        shadow.tint = 0x010101;

        var name = this.add.bitmapText(x,y, "JandaManateeSolid", letter, 70);
    ...
    showCorrectName: function(y, gotoNext, dontHide) {

        var itens = [];

        for(var i = 0; i < this.nameCorrect.length; i++) {
            if(this.nameTexts[i].text == "_") {
                itens = this.addLetter(this.nameCorrectPos, y, this.nameCorrect[i]);
            }
        }
        ...
        if(gotoNext) {
            this.add.tween(this.groupName).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true, 1000);
            this.add.tween(this).to({}, 500, Phaser.Easing.Linear.None, true, 1500).onComplete.add(this.gotoNextLevel, this);
        } else {
            this.add.tween(this.groupName).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true, 1000);
            if(dontHide == null || dontHide == false) {
                this.hideAndShowLevel(false);
            }
        }
    },
    ...
    createScene: function() {//finished
        ...
        this.add.sprite( -560, -156, 'background');
        this.createAnimation( 128, 320, 'pi1', 1,1);
        this.createAnimation( 705, 345, 'pi2', 1,1);

        this.createAnimation( 425, 458, 'red1', 1,1);
    ...
    showLiveTutorial: function() {
        ...
        this.teclado = this.add.sprite(this.world.centerX, 50, "teclado");
        this.teclado.anchor.set(0.5,0);

        this.groupIntro.add(this.teclado);

        this.nameCorrect = "FOCA";
        this.showName(300, "_OCA");

        this.groupIntro.add(this.groupName);

        this.createDelayTime( 8000, function() {
            
            var lf = this.add.sprite(this.world.centerX-100, 110, "teclaF");
            lf.anchor.set(0.5,0.5);
            lf.scale.set(0.5,0.5);

            var lv = this.add.sprite(this.world.centerX+100, 110, "teclaV");
            lv.anchor.set(0.5,0.5);
            lv.scale.set(0.5,0.5);

            this.groupIntro.add(lf)
            this.groupIntro.add(lv);

            this.add.tween(lf).to({alpha: 1}, 500, Phaser.Easing.Linear.None, true);
            this.add.tween(lv).to({alpha: 1}, 500, Phaser.Easing.Linear.None, true);

            this.add.tween(lf.scale).to({x: 1, y: 1}, 500, Phaser.Easing.Linear.None, true);
            this.add.tween(lv.scale).to({x: 1, y: 1}, 500, Phaser.Easing.Linear.None, true);


            this.createDelayTime(4500, function() {
                this.add.tween(lf.scale).to({x: 1.2, y: 1.2}, 600, Phaser.Easing.Linear.None, true);
                this.add.tween(lf).to({alpha: 0.7}, 200, Phaser.Easing.Linear.None, true, 0, 5, true);

                this.showCorrectName(300,false, true);
            });


            this.createDelayTime(12500, function() {
                this.add.tween(lf).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);
                this.add.tween(lv).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);
    ...
    showNextLevel: function() {
        ...
        if(this.groupName != null) {
            ...
            this.groupName.alpha = 1;
        }

        switch(this.currentLevel) {
            case 1:
                this.numCorrects = null;
                if(!this.isWrong) {
                    this.sound.play("soundP");
                }
                this.initLevel(["fada", "vara", "foca"]);
            break;
            case 2:
                this.numCorrects = null;
                if(!this.isWrong) {
                ...
                this.initLevel(["vaca", "faca", "voar"]);
            break;
            case 3:
                if(this.numCorrects == null) {
                    this.numCorrects = 1;
                    if(!this.isWrong) {
                        this.sound.play("soundP");
                    }
                    this.initLevel(["bife", "cavalo", "tofu"]);
                } else {
                    this.numCorrects=null;
                    if(!this.isWrong) {
                        this.sound.play("soundP");
                    }
                    this.initLevel(["cravo", "bravo", "tufo"]);
                }
            break;
        }
        this.isWrong = false;
    ...
    onKeyPressed: function(elem) {
        ...
        if(elem.toLowerCase() == this.correctLetter) {
    ...
    initLevel: function(nomes) {
        ...
        this.createDelayTime((this.isWrong)?500:1500, function() {
            if(!this.isWrong) {
                this.sound.play("sound_" + this.nameCorrect); 
            }

            if(item.indexOf("f")>= 0) {
                this.correctLetter = "f";
                item = item.replace("f", "_");
            } else {
                this.correctLetter = "v";
                item = item.replace("v", "_");
            }

            this.showName(300, item);
    ...
    clickRightButton: function(target) {
        ...
        this.showCorrectName(300,this.numCorrects==null);
