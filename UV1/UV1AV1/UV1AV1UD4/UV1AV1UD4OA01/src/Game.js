...
BasicGame.Game.prototype = {
    ...
    showTextoIntro: function() {
        ...
        this.createDelayTime(12500, function() {
            ...
            this.createDelayTime(13000, function() {
                this.arrow = this.add.sprite(this.world.centerX+100, this.world.centerY-30, "mouseIntro");
                this.arrow.anchor.set(0.5,0.5);

                this.groupIntro.add(this.arrow);

                this.add.tween(this.arrow).to({x:this.world.centerX+120, y: this.world.centerY+20}, 1200, Phaser.Easing.Linear.None, true).onComplete.add(this.showClickEffect,this);
            })
        });
    ...
    showClickEffect: function() {
        var clique = this.add.sprite(this.world.centerX+100, this.world.centerY-10, 'efeitoClique');
    ...
    showBumbaFixo: function(){
        this.bumbaFixo = this.createAnimation( 700, 200, 'bumbaFixo', 1,1);
    },
    ...
    showNextLevel: function() {
        ...
        switch(this.currentLevel) {
            case 1:
                ...
                this.initLevel1((this.isWrong)?500:3000);
            break;
            case 2:
                if(!this.isWrong) {
    ...
    initLevel1: function(delay) {
        this.itens = ['soundF-3','soundF-4','soundF-5'];

        this.numCorrects = 1;
        this.maxErrors = 1;

        var audio = this.createRandomItens(this.itens, this.numCorrects);

        this.createDelayTime(delay, function() {
            this.questionAudio = this.sound.play(audio);
            this.buttons = [];
            var correto = false;
			
			for(var i = 0; i < 10; i++) {
				if(String(audio).indexOf(i+1) >= 0) {
					correto = true;
					this.numPrfCorreto = i+1;
					console.log(this.numPrfCorreto);
				}else{
					correto = false;
				}
				this.buttons.push( this.createButton(210 + 65*i, this.world.centerY+30, "parafuso" + (i+1), correto, 500) );
    ...
    initLevel2: function(delay) {
        this.itens = ['soundM-6','soundM-7','soundM-8'];
    ...
    efeitoDesparafusar: function() {

        this.clearButtons(false);

        this.add.tween(this.buttons[this.numPrfCorreto - 1]).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);

        var prfAtual = this.add.sprite(this.correctItem.x - 45, this.correctItem.y - 65, "efParafuso" + this.numPrfCorreto);
        prfAtual.animations.add('idle', null, 18, false);
        prfAtual.animations.play('idle');
        prfAtual.scale.set( 1, 1);

        this.createDelayTime( 3000, function() {
            this.add.tween(prfAtual).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);
            this.gotoNextLevel();
            //this.addPoints();
        });
    },
	...
	update: function () {
		...
		if(this.questionAudio != null){
			if(!this.questionAudio.isPlaying){
				for( var i = 0; i < 10; i++){
					this.buttons[i].input.enabled = true;	
				}
