...
BasicGame.Game.prototype = {
    ...
    gameOverMacaco: function() {
        ...
        var key = this.add.sprite(this.world.centerX, this.game.height, "chave");
        key.anchor.set(1,0.5);
        key.alpha = 0;

        this.add.tween(key).to({alpha: 1, y: 280}, 1000, Phaser.Easing.Linear.None, true);
        this.add.tween(key.scale).to({x: 0.3, y: 0.3}, 1000, Phaser.Easing.Linear.None, true);

        this.createDelayTime(1400, function() {

            this.add.tween(key).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true).onComplete.add(function() {
                this.porta.frame = 1;
            }, this);
        });
    ...
    createScene: function() {//finished
        ...
        this.add.sprite( -55, -36, 'background');
        this.porta = this.add.sprite(this.world.centerX,40,"porta", 0);
        this.porta.anchor.set(0.5,0);
    },

    // tutorial demonstracao - inicio
    showLiveTutorial: function() {

        var teclado = this.add.sprite(this.world.centerX, 100, "teclado");
        teclado.anchor.set(0.5,0.5);

        this.groupIntro.add(teclado);

        this.createPis([2]);

        for(var i = 0; i < this.pis.length;i++) {
            this.groupIntro.add(this.pis[i]);
        }

        this.createDelayTime( 6000, function() {
            
            var tecla = this.add.sprite(this.world.centerX, 80, "tecla");
            tecla.anchor.set(0.5,0.5);
            tecla.alpha = 0;
            tecla.scale.set(0.3,0.3);

            this.groupIntro.add(tecla);

            this.add.tween(tecla).to({alpha:1}, 500, Phaser.Easing.Linear.None, true);
            this.add.tween(tecla.scale).to({x:1, y: 1}, 500, Phaser.Easing.Linear.None, true);

            this.add.tween(this.pis[1]).to({alpha:1}, 500, Phaser.Easing.Linear.None, true);

            this.createDelayTime(2500, function() {

                for(var i = 0; i < this.pis.length; i++) {
                    this.add.tween(this.pis[i]).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);
                }
                this.add.tween(tecla).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);
                this.add.tween(teclado).to({alpha: 0}, 500, Phaser.Easing.Linear.None, true);
            });

            this.add.tween(this.tutorialPlacar).to({y: -300}, 500, Phaser.Easing.Linear.None, true, 3000).onComplete.add(this.initGame, this);
    ...
    initLevel: function() {
        ...
        this.itens = this.createRandomItens([1,2,3,4,5,6,7,8,9], 5);
        
        this.createPis(this.itens);

        this.initKeyboard();

    },

    createPis: function(elems) {
        var pos = [null,
            [  0,305],
            [100,315],
            [185,295],
        ...
        for(var i = 1; i <= 10; i++) {
            ...
            var p = this.createAnimation(pos[i][0], pos[i][1], "pi" + i, 1,1);
            this.pis.push(p);
            var n = p.animations.currentAnim._frames.length-1;
            p.animations.currentAnim.frame = this.rnd.integerInRange(0,n);
            p.animations.play("idle");

            if(elems.indexOf(i) >= 0) {
                p.alpha = 0;
            }
        }
        return this.pis;
    ...
    onKeyPressed: function(elem) {
        ...
        if(p>=0) {
            ...
            this.add.tween(this.pis[n-1]).to({alpha: 1}, 500, Phaser.Easing.Linear.None, true);
