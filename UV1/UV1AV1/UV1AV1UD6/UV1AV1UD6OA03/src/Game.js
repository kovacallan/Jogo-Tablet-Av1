...
BasicGame.Game.prototype = {
    ...
    showTextoIntro: function() {
        ...
        this.createDelayTime( this.TEMPO_INTRO, function() {
            ...
            tutorialText2 = this.add.sprite( this.world.centerX, 110, 'initialText2');
    ...
    hideLevel: function(callback) {
        ...
        if(callback != null) {
            this.add.tween(this).to({}, 800, Phaser.Easing.Linear.None, true, 500).onComplete.add(callback, this);
        } else {
            this.add.tween(this).to({}, 800, Phaser.Easing.Linear.None, true, 500);
        }
    },
    ...
    createScene: function() { //finished
        ...
        this.add.sprite( -325, -55, 'background');
        //this.createAnimation( -150, 265, 'bumba', 1,1);

        this.player = this.add.group();
        var p1 = this.createAnimation(115, 280, 'passaro1', 1,1);
        var p2 = this.createAnimation( 80, 350, 'passaro2', 1,1);
        ...
        //this.listObstacles = this.add.group();
        //this.addObstacle();
    ...
    initLevel1: function() {
        ...
        this.itens = ["nuvem", "poste", "folhas"];
        this.numItens = 5;
            

        var _letters = []
        for(var i = 0; i < this.numItens; i++) {
            var n = this.rnd.integerInRange(0,this.itens.length-1);
            _letters.push( this.itens[n] );
        }
        ...
        for(var i = 0; i < _letters.length; i++) {
            this.addObstacle(_letters[i]);
        }

        this.initKeyboard();
    },

    addObstacle: function(name) {
        ...
        var ob1 = this.add.sprite(this.iniX, py, name, 0, this.listObstacles);
        this.listMoves.push(ob1.width + 300);
        this.iniX += (ob1.width + 300);
        ob1.anchor.set(0,1);
    },
    ...
    initKeyboard: function() {
        ...
        upkey.onDown.add(this.onKeyUp, this);

        var downkey = this.input.keyboard.addKey(Phaser.Keyboard.DOWN);
        downkey.onDown.add(this.onKeyDown, this);
    ...
    onKeyUp: function() {
        ...
        console.log("Phaser.Keyboard.UP");
        this.verifyMovement(Phaser.Keyboard.UP);

    },
    onKeyDown: function() {
        this.removeKeyboard();

        console.log("Phaser.Keyboard.DOWN");
        this.verifyMovement(Phaser.Keyboard.DOWN);

    },

    verifyMovement: function(dir) {

        var _name = this.listObstacles.getAt(this.currentItem).key;
        ...
        switch(dir) {
            case Phaser.Keyboard.UP:
                ...
                isRight = (_name == "poste" || _name == "folhas");
                py = -300;
        ...
        if(isRight) {
            ...
            this.add.tween(this.player).to({y:py}, 500, Phaser.Easing.Linear.None, true, 0, 0, true);
            this.clickRightButton();

        } else {

            this.add.tween(this.player).to({y: -30}, 70, Phaser.Easing.Linear.None, true, 0, 6, true);
    ...
    onCompleteMove: function() {
        ...
        if(this.currentItem >= this.numItens) {
            this.gameOverMacaco();
        } else {
            this.initKeyboard();
        }
    ...
    clickRightButton: function(target) {
        ...
        var px = this.listObstacles.x-this.listMoves[this.currentItem];
        this.add.tween(this.listObstacles).to({x: px}, 1000, Phaser.Easing.Linear.None, true).onComplete.add(this.onCompleteMove, this);
        this.currentItem++;
    },
